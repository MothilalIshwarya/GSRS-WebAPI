...
{
    public class LicenseTypeRepository : GSRSRepository<LicenseType>, ILicenseTypeRepository
    {
        #region Constructor
        private readonly IBaseContext context;
        public LicenseTypeRepository(IContext _context)
            : base(_context)
        { 
            context = _context;
        }
        #endregion
        protected virtual IQueryable<LicenseType> Query
        {

            get { return context.SetQuery<LicenseType>(); }
        }

        //getall override method
        public override Task<Dictionary<int, LicenseType>> SearchUnrestrictedAsync(ISearchCondition<LicenseType> searchCondition)
        {
            ...
            if (searchCondition != null)
            {
                ...
                entities = Query.Include(x=>x.ImportType);
            }
        ...
        public override Task<LicenseType> FetchAsync(int id, bool isTracked = true)
        {
        ...
        private Task<LicenseType> FetchEntityByIdAsync(int id)
        {
            // Query includes tenant check
            //var result = Query.Include(x => x.ProductGroup).FirstOrDefaultAsync(x => x.Id == id);
            //var result = Query.Include("LicenseType.ProductGroup").Include("LicenseType.LicServerGroup").Where(x => x.Id == id).ToList();
            var entities = Query.Include(x=>x.ImportType);
